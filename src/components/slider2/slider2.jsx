        ...
        <article className='slider2'>
            ...
            <div className='slider2__list'>
                {technology.map((tech, index) => (
                    <button className={`slider2__list-button${technoSelected === index ? ' slider2__list-button--selected' : ''}`} key={tech.name} onClick={ () => {setTechnoSelected(index); setTechno(index)}}>{index + 1}</button>
                ))}
            </div>
